...
router.post("/", auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        const populated = await Booking_1.default.findById(savedBooking._id)
            .populate("room")
...
router.get("/", auth_1.authenticate, (0, auth_1.requireRole)(["admin", "staff"]), async (_req, res) => {
    try {
...
router.put("/:id", auth_1.authenticate, (0, auth_1.requireRole)(["admin", "staff"]), async (req, res) => {
    ...
    try {
        ...
        const updatedBooking = await Booking_1.default.findByIdAndUpdate(req.params.id, { status }, { new: true })
            .populate("room")
            .populate("user", "name email");
        if (!updatedBooking) {
